/*
	...
	* Author   : *
	* Mail     : *
	* Creation : Wed 07 Apr 2021 08:22:34 PM CST
*/
...
std::string Inspector::getMemDiskInfo() {
    ...
    rapidjson::Value cpuObj(rapidjson::kObjectType);//创建一个Object类型的元素
    cpuObj.AddMember("used_rate", cpurate(), allocator);
    document.AddMember("cpu", cpuObj, allocator);


    rapidjson::Value memObj(rapidjson::kObjectType);//创建一个Object类型的元素
    memObj.AddMember("total_memory", meminfo_.total, allocator);
    memObj.AddMember("memory_used_rate", meminfo_.usedRate, allocator);
    document.AddMember("memory", memObj, allocator);

    rapidjson::Value diskObj(rapidjson::kObjectType);//创建一个Object类型的元素
...
//cpu use rate
double Inspector::cpurate() {
    double rate = calculateCpuRate();
    return (rate);
}

void Inspector::getCpuInfo() {
    lastCpuInfo = nowCpuInfo;
    ...
    std::sscanf(buf, "%s %u %u %u %u %u %u %u", reinterpret_cast<char *>(&nowCpuInfo.names),
...
double Inspector::calculateCpuRate() {
    double cpuUsed = calculateCpuRateUtil(lastCpuInfo, nowCpuInfo);
    //LOG_INFO << "cpu used" << cpuUsed;
    return cpuUsed;
}

void Inspector::MEMINFO() {
    meminfo_ = getMemInfo();
}

void Inspector::DISKINFO() {
    diskinfo_ = getDiskInfo();
}
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  else
  {
      ...
      else if (result.size() == 1)
      {
          ...
          if(result[1] == "info") {
              std::vector<std::string> v = split(req.path());
              string body = getMemDiskInfo();
              resp->setStatusCode(HttpResponse::k200Ok);
              resp->setStatusMessage("OK");
