...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  if (req.path() == "/")
  {
    ...
    resp->addHeader("Server", "junpyu");
    string now = Timestamp::now().toFormattedString();
	std::string body("open");
	std::string fileName_("index.html");
	int src_fd = open(fileName_.c_str(), O_RDONLY, 0);
	if (src_fd < 0) { 
		std::cout << "open error" << std::endl;
		resp->setBody("<html><head><title>This is title</title></head>"
        "<body><h1>open error</h1>Now is " + now +
        "</body></html>");
		return;
	}
	auto len = lseek(src_fd,0,SEEK_END);
    void *mmapRet = mmap(NULL, len, PROT_READ, MAP_PRIVATE, src_fd, 0);
    close(src_fd);
    char *src_addr = static_cast<char *>(mmapRet);
    body = std::string(src_addr, src_addr + len);
    munmap(mmapRet, len);
...
int main(int argc, char* argv[])
{
  ...
  EventLoop loop;
  HttpServer server(&loop, InetAddress(8000), "junpyu");
